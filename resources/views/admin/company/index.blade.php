                    ...
                    <table class="table table-striped">
                        ...
                        <tbody>
                         @forelse ($companies as $company)
                            <tr><td> <img src="{{asset('storage/'.$company->logo)}}" width="80" /></td>
                                <td>{{$company->name}}</td>
                                <td>{{$company->email}}</td>
                                <td>{{$company->website}}</td>
                                <td> 
                                    <a href="{{route('company.edit', [$company->id])}}" class="text-blue-500"><button class="btn btn-primary">Edit</button></a>
                                    <form method="post" action="{{route('company.destroy', [$company->id])}}" id="deleteForm{{$company->id}}">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="btn btn" onclick="event.preventDefault(); if(confirm('Are you sure to delete?')) {document.getElementById('deleteForm{{$company->id}}').submit();} else {return false;} ">Delete</button>
                                    </form>
                                </td>
                            </tr>
                            @empty
